...
(same as htlcLimiter but with fixed fee group for all htlcs = static limitation)
...
const ALLOWED_PER_GROUP_IN = group => group + 6 // how many incoming htlcs allowed per fee group
const ALLOWED_PER_GROUP_OUT = group => group + 6 // how many outgoing htlcs allowed per fee group
...
//const ALLOWED_BELOW_LIMIT_IN = 4 // at most _ utxo<SATS_LIMIT should end up being settled on chain out of inward ones
//const ALLOWED_BELOW_LIMIT_OUT = 8 // at most _ utxo<SATS_LIMIT should end up being settled on chain out of outward ones
...
const decideOnForward = ({ f }) => {
  ...
  try {
    ...
    const allowed = inboundFeeGroupCount <= inboundFeeGroupLimit && outboundFeeGroupCount <= outboundFeeGroupLimit
...
const getFee = (f, channelUpdate = false, foundId = null) => { 
  ...
  // put them into the same fee group
  return 0 
